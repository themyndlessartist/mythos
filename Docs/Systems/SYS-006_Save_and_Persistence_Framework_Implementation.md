...
M-002 extends the same orchestration with a required Relationship partition restored after Entity identity. The framework prototype marker is `m-002.0`; relationship IDs, participants, kinds, lifecycle, canonical dimensions, timestamps, and provenance survive complete-world round trips. This extension does not select a production migration or storage policy.
...
The Reputation increment adds a required Reputation partition restored after Entity identity and advances the prototype marker to `m-002.3`. Audience-scoped values, lifecycle, timestamps, provenance, and Entity references remain deterministic and integrity checked.
