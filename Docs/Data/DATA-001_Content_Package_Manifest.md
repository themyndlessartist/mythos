...
| `schema_version` | string | DATA-001 major/minor version; `1.0`, `1.1`, or backward-compatible `1.2` |
...
Each `entries` item contains `kind` (`character`, `npc`, `sprite-animation`, `layered-map`, `settlement-project`, or `asset`), stable `id`, unique package-relative `path`, `media_type`, byte `size`, and `integrity` (`algorithm`, `digest`). The `character` kind is available beginning with schema `1.1`; `settlement-project` begins with `1.2`. Earlier packages remain valid but cannot use later entry kinds. Record IDs must match the ID inside the referenced document. Entries are ordered by `kind`, then ID, then path for deterministic export.
...
- Every DATA-002 through DATA-006 record and referenced asset is declared exactly once.
