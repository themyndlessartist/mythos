...
public sealed class WorldPersistence(ISaveStorage storage)
{
    ...
    public PersistenceResult<PersistentWorldState> Load(string slotId, PersistenceLoadContext context)
    {
        ...
        try
        {
            ...
            var propertyData = Get<PropertyFrameworkSnapshot>(partitions, "properties");
            if (!propertyData.IsSuccess) return Fail<PersistentWorldState>(propertyData.Error!);
            var propertyRestore = properties.RestoreSnapshot(propertyData.Value);
            ...
            var economyData = Get<EconomyFrameworkSnapshot>(partitions, "economy");
            if (!economyData.IsSuccess) return Fail<PersistentWorldState>(economyData.Error!);
            var economyRestore = economy.RestoreSnapshot(economyData.Value);
